    ...
    function bindNavMirrorSearch() {
      ...
      const isSermonModeActive = () => sidebarTab === 'bible'
        && typeof isActiveSermonCollection === 'function'
        && isActiveSermonCollection();
      ...
      const refreshNavMirrorResults = () => {
        ...
        // Sermon collections: this box searches paragraph content independently of
        // whatever is typed in the sidebar's title-only search — it does not read
        // from #song-list at all in that mode.
        if (isSermonModeActive()) {
          const contentItems = buildSermonContentMatchItems(mirror.value);
          if (!contentItems.length) { hideMirrorResults(); return; }
          const visible = contentItems.slice(0, 40);
          resultsBox._sourceItems = visible;
          ...
          visible.forEach((srcItem, i) => {
            const row = document.createElement('div');
            row.className = 'song-item nav-mirror-result-item';
      ...
      // Sync mirror → main on input — except in sermon-collection mode, where the
      // mirror is an independent content search and must not overwrite the sidebar's
      // own title search value/results.
      mirror.addEventListener('input', () => {
        ...
        // In sermon mode this triggers a full-text scan across the entire
        // sermon collection (buildSermonContentMatchItems/findBibleKeywordMatches
        // — 200k+ paragraphs) — measured at up to ~110ms per call. Running
        // that on every single keystroke with no debounce is what was
        // freezing the app for a moment while typing here.
        clearTimeout(mirrorSearchTimer);
        mirrorSearchTimer = setTimeout(refreshNavMirrorResults, 150);
      });
      ...
      // Sync main → mirror on input — except in sermon-collection mode (see above).
    ...
    if (ui.dualVersionSecondaryId != null) {
      ...
      if (ui.bgUploadDataUrl != null) bgUploadDataUrl = sanitizeStoredBgMediaUrl(ui.bgUploadDataUrl, BG_IMAGE_MAX_DATA_URL_CHARS);
      ...
      if (ui.bgVideoUploadDataUrl != null) bgVideoUploadDataUrl = sanitizeStoredBgMediaUrl(ui.bgVideoUploadDataUrl, BG_VIDEO_MAX_DATA_URL_CHARS);
    ...
    function saveState(label) {
      const state = {
        ...
        // bibles/sermons is large (100MB+ once a sermon collection is
        // loaded) and, unlike songs, is never mutated in place anywhere in
        // the app — every place that changes it replaces the whole
        // bibles[version] array with a new one (import, reseed, delete)
        // rather than editing an existing array's contents. That makes a
        // shallow copy exactly as safe as a deep clone here, for a fraction
        // of the cost. Deep-cloning it on every saveState() call (undo
        // history keeps up to 50 snapshots) was adding ~2.5s per call and
        // was the actual cause of the app eventually running out of memory
        // and crashing ("Render process gone") during long sessions.
        bibles: { ...bibles },
      ...
      // Capped conservatively — each entry still carries its own clone of the
      // full songs array (bibles/schedule are now cheap references, but
      // songs is not, since individual song objects do get edited in place
      // elsewhere and must stay independent per snapshot).
    ...
    function applyLoadedState(stateValue, songRecords, bibleRecords, opts = {}) {
      ...
      bibleRecords.forEach(record => {
        ...
        if (record.collectionType) bibleVersionMeta[record.id] = { collectionType: record.collectionType };
      });
    ...
    // Pre-loads the bundled sermon/song collections shipped with the desktop
    // installer (app-src/bundled-data/) so a fresh install already has content,
    // instead of requiring a manual Import right after setup. Runs on every
    // launch (not just the first) and merges additively by title/book name —
    // any bundled song or sermon not already present gets added, but nothing
    // already in the library is ever overwritten or removed. This is what lets
    // a reinstalled/updated .exe with new bundled content (e.g. newly added
    // songs) reach an existing installation automatically, without requiring
    // the user to manually re-import — while never touching anything the user
    // themselves imported, edited, or deleted.
    function showFirstRunLoadingOverlay(show) {
      let el = document.getElementById('first-run-loading-overlay');
      if (show && !el) {
        el = document.createElement('div');
        el.id = 'first-run-loading-overlay';
        el.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(10,12,16,0.96);color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:inherit;gap:14px;';
        el.innerHTML = '<div style="width:40px;height:40px;border-radius:50%;border:3px solid rgba(255,255,255,0.25);border-top-color:#fff;animation:bsp-firstrun-spin 0.8s linear infinite"></div>'
          + '<div style="font-size:15px;opacity:0.9">Préparation des sermons et chansons — un instant…</div>'
          + '<style>@keyframes bsp-firstrun-spin{to{transform:rotate(360deg)}}</style>';
    ...
    // One-time cleanup for a known encoding bug in an earlier bundled build:
    // for songs from "CANTIQUES JRT" where the PowerPoint title and lyrics sat
    // on the same slide, the old encoder sometimes captured a long run-on
    // string (title + lyrics glued together) as the title. Any such song
    // already imported into this profile is found by an EXACT (title, text)
    // match against the known bad output and removed; seedBundledDataIfEmpty
    // (called right after) then adds the corrected song back in, or silently
    // skips it if a proper version already exists under its real title.
    async function applyCantiquesJrtCorrections() {
      try {
        const flag = await idbGet(STORE_STATE, 'cantiquesJrtCorrectionsApplied');
        const CORRECTIONS_VERSION = 'cantiques-jrt-title-fix-2026-07-29';
        if (flag && flag.value === CORRECTIONS_VERSION) return;

        const resp = await fetch('bundled-data/cantiques-jrt-corrections.json').catch(() => null);
        if (!resp || !resp.ok) return;
        const data = await resp.json();
        if (!data || data.type !== 'bsp-song-corrections' || !Array.isArray(data.corrections)) return;

        const byOldPair = new Map();
        data.corrections.forEach((c) => {
          if (c && c.oldTitle && c.oldText) byOldPair.set(c.oldTitle + ' ' + c.oldText, c);
        });
        if (!byOldPair.size) return;

        let removed = 0;
        const kept = [];
        for (const s of (Array.isArray(songs) ? songs : [])) {
          const match = byOldPair.get((s.title || '') + ' ' + (s.content || s.text || ''));
          if (match) {
            await idbDelete(STORE_SONGS, s.id).catch(() => {});
            removed++;
          } else {
            kept.push(s);
          }
        ...
        await idbPut(STORE_STATE, { key: 'cantiquesJrtCorrectionsApplied', value: CORRECTIONS_VERSION, updatedAt: Date.now() }).catch(() => {});
      } catch (e) {
        console.error('CANTIQUES JRT title correction failed', e);
      }
    }

    async function seedBundledDataIfEmpty() {
      try {
        // Cheap existence checks first (title/version-name only) so a normal
        // launch where nothing new needs adding doesn't pay for fetching or
        // parsing the bundled files at all.
        const existingSongTitles = new Set(
          (Array.isArray(songs) ? songs : []).map(s => normalizeSearchText(s.title || ''))
        );
        const existingBibleVersions = new Set(Object.keys(bibles || {}));

        const [songResp, sermonResp] = await Promise.all([
          fetch('bundled-data/songs.json').catch(() => null),
          fetch('bundled-data/sermons.json').catch(() => null)
        ]);
        if (!(songResp && songResp.ok) && !(sermonResp && sermonResp.ok)) return;

        // Parsing the ~135MB bundled sermon file blocks the renderer for a
        // noticeable stretch on every launch (its version name can't be
        // cheaply peeked at without parsing the whole thing) — show a
        // full-screen overlay first, and yield a frame so it actually paints
        // before the heavy synchronous parse starts, so this reads as
        // "loading", not a frozen/broken app.
        showFirstRunLoadingOverlay(true);
        await new Promise((resolve) => requestAnimationFrame(() => setTimeout(resolve, 0)));

        let addedSongs = 0;
        let addedSermonVersion = false;

        if (songResp && songResp.ok) {
          const songData = await songResp.json();
          if (songData && songData.type === 'bsp-song-collection' && Array.isArray(songData.songs)) {
            const incomingSongs = songData.songs.filter(s => s && s.title && (s.text || s.content));
            for (const s of incomingSongs) {
              const titleKey = normalizeSearchText(s.title || '');
              if (existingSongTitles.has(titleKey)) continue;
              existingSongTitles.add(titleKey);
              ...
              songs.push(newSong);
              await idbPut(STORE_SONGS, buildSongRecord(newSong, { isNew: true })).catch(() => {});
              addedSongs++;
            }
          }
        }

        if (sermonResp && sermonResp.ok) {
          // The version name lives inside the JSON body, so there's no way to
          // check it without parsing the whole file first — the check right
          // after is what actually skips re-adding an already-present version.
          const bulkData = await sermonResp.json();
          if (bulkData && bulkData.type === 'bsp-sermon-collection' && Array.isArray(bulkData.books)) {
            ...
            if (!existingBibleVersions.has(versionName)) {
              const incoming = bulkData.books.filter(b => b && b.book && b.content);
              bibles[versionName] = incoming.map(b => ({ ...b, version: versionName }));
              ...
              await idbPut(STORE_BIBLES, buildBibleRecord(versionName, bibles[versionName], { isNew: true, collectionType: 'sermon' })).catch(() => {});
              addedSermonVersion = true;
            }
          }
        }

        if (!addedSongs && !addedSermonVersion) return;

        saveState();
        if (typeof renderVersionBar === 'function') renderVersionBar();
        if (typeof updateBibleLists === 'function') updateBibleLists();
        if (typeof renderSongs === 'function') renderSongs();
        if (typeof ensureSelectionFallback === 'function') ensureSelectionFallback();
      } catch (e) {
        console.error('Bundled data seeding failed', e);
      } finally {
        showFirstRunLoadingOverlay(false);
      }
    }
    ...
    async function bootApp() {
      ...
      await applyCantiquesJrtCorrections();
      await seedBundledDataIfEmpty();
