    ...
    function saveCurrentItem() {
      ...
      // saveState() alone deep-clones the entire songs/bibles/schedule
      // collections via JSON (for undo/redo history) — with 2000+ songs and a
      // large sermon collection loaded, doing that (plus a full lyric-buttons
      // rebuild) on every single keystroke made correcting a song's text
      // freeze the app for the duration of that clone, repeated per
      // character. Debounced instead: still captured for undo, just once
      // after the user actually pauses typing. The lyric-buttons list is
      // hidden while in text-editing mode anyway, so there's nothing lost by
      // not rebuilding it on every keystroke.
      clearTimeout(saveCurrentItemHeavyTimer);
      saveCurrentItemHeavyTimer = setTimeout(() => {
        saveState();
        // The lyric-buttons list is display:none while in text-editing mode
        // (measured ~135ms to rebuild, entirely wasted since it's not on
        // screen) — skip it here and let setEditorMode() refresh it once,
        // right when switching back to button mode instead.
        if (editorMode !== 'text') updateButtonView();
    ...
    function buildReferenceBadgeSpan(text, fontSizePt, foregroundColor, backgroundColor, opts = {}) {
      // white-space:normal (not nowrap) + a max-width lets long sermon titles wrap onto
      // a second line instead of overflowing off-screen — short references still render
      // on one line since they never reach the max-width. The LT container height is
      // grown to match via calculateLtHeightPctFromLines's referenceCount argument (see
      // pushLiveUpdate), so a wrapped 2nd line no longer gets clipped/pushed off-screen.
      const styles = [
    ...
    function setEditorMode(m) {
      ...
      const wasText = editorMode === 'text';
      editorMode = m;
      // saveCurrentItem() skips rebuilding the (hidden) button list while in
      // text mode, so it can be stale by the time we switch back to it —
      // refresh it here instead, right as it becomes visible again.
      if (m === 'btn' && wasText) updateButtonView();
    ...
    function deleteItem(i, e) {
      ...
      if (sidebarTab === 'songs') {
        ...
        schedulePersistBibleVersion(activeBibleVersion);
      }
      saveState();
      saveToStorageDebounced();
      renderSongs();
