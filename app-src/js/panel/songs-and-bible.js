    ...
    function getPagesCacheKey(item, isBible, lineCountOverride) {
      ...
      if (isBible) {
        const isSermonCollection = !!(bibleVersionMeta[item.version] && bibleVersionMeta[item.version].collectionType === 'sermon');
        const sermonWrapKey = isSermonCollection
          ? [
              ltFontBible,
    ...
    // A dedicated title-only page shown before a song's lyrics start, so the
    // title never has to share the screen with (or be confused with) the
    // lyrics themselves.
    // Fixed size for the title page, independent of whatever font size is
    // configured/auto-resized for the lyrics themselves.
    const SONG_TITLE_PAGE_FONT_SIZE_PT = 65;

    function buildSongTitlePage(item) {
      const title = String((item && item.title) || '').trim();
      if (!title) return null;
      return {
        text: `<span style="font-size:${SONG_TITLE_PAGE_FONT_SIZE_PT}pt">${renderSongPageHtml([title])}</span>`,
        raw: title,
        tag: 'Titre',
        isTitlePage: true
      };
    ...
    function getPagesFromItem(item, isBible, lineCountOverride = null) {
      ...
      if (!isBible) {
        ...
        if (shouldDisplaySongsBySection()) {
          const sectionPages = buildSongSectionPages(filteredLines, categoryTag || tag);
          const titlePage = buildSongTitlePage(item);
          return setBoundedCacheValue(
            ITEM_PAGES_CACHE,
            cacheKey,
            titlePage ? [titlePage, ...sectionPages] : sectionPages,
            ITEM_PAGES_CACHE_LIMIT
          );
        }
        ...
        const titlePage = buildSongTitlePage(item);
        return setBoundedCacheValue(ITEM_PAGES_CACHE, cacheKey, titlePage ? [titlePage, ...pages] : pages, ITEM_PAGES_CACHE_LIMIT);
      } else {
        const pages = [];
        let tag = "Scripture";
        const verseLines = [];
        ...
        const isSermonCollection = !!(bibleVersionMeta[item.version] && bibleVersionMeta[item.version].collectionType === 'sermon');
        // Sermon paragraphs are long prose, not short verses: group one at a time and
        // let pushVerseGroupPages() below split it into effectiveLinesPerPage-sized
        // physical-line chunks instead of grouping several paragraphs per screen.
        const groupSize = isSermonCollection ? 1 : effectiveLinesPerPage;
        ...
        const wrapFontFamily = document.getElementById('font-family')?.value || '';
        ...
        const pushVerseGroupPages = (slice) => {
          ...
          if (isSermonCollection && slice.length === 1) {
            const verseLine = slice[0];
            const match = verseLine.match(/^(\d+)\s+(.+)$/);
            const verseNum = match ? match[1] : getSliceStartVerse(slice);
            const verseText = match ? match[2] : verseLine;
            const wrapped = wrapVerseTextIntoLines(verseText, wrapTextWidthPx, ltFontBible, wrapFontFamily, wrapFontWeight);
          ...
          pages.push({
            text: combineVersesIntoFlow(slice),
            raw: slice.join('\n'),
            tag,
        ...
        if (startIndices.length && startIndices[0] > 0) {
          pushVerseGroupPages(verseLines.slice(0, startIndices[0]));
        }
        startIndices.forEach((idx) => {
          pushVerseGroupPages(verseLines.slice(idx, idx + groupSize));
        });
        let nextStart = startIndices.length ? (startIndices[startIndices.length - 1] + groupSize) : 0;
        while (nextStart < verseLines.length) {
          pushVerseGroupPages(verseLines.slice(nextStart, nextStart + groupSize));
          nextStart += groupSize;
        }
    ...
    function getBibleRefForPage(item, pageRaw, verseCount = 1) {
      ...
      const isSermon = !!(bibleVersionMeta[item.version] && bibleVersionMeta[item.version].collectionType === 'sermon');
      const displayBookLabel = isSermon ? formatSermonBookLabel(bookLabel) : bookLabel;
      const baseTitle = (book && chap)
        ? (isSermon ? displayBookLabel : `${bookLabel} ${chap}`)
      ...
      if (book && chap && firstVerseNo) {
        ...
        const refSep = isSermon ? ' §' : ` ${chap}:`;
        if (firstVerseNo !== lastVerseNo) return `${displayBookLabel}${refSep}${firstVerseNo}-${lastVerseNo}`;
        return `${displayBookLabel}${refSep}${firstVerseNo}`;
      }
      return baseTitle;
    }
    ...
    function getEffectiveLiveBackgroundState() {
      const panelKind = getCurrentPanelContentKind();
      const viewingLiveTab = !!(isLive && livePointer && panelKind && livePointer.kind === panelKind);
      if (viewingLiveTab) {
        liveBackgroundState = captureBackgroundStateFromUi();
        // The panel currently showing IS the live one — the freshly captured
        // live UI state above is authoritative and must win. The per-tab
        // "profile" snapshot below is a fallback for when the user has
        // navigated away from the live tab (so its own controls aren't the
        // ones currently on screen); it's only refreshed on tab-switch, so
        // using it here would keep re-sending whatever background was active
        // the last time the user switched tabs — never picking up in-place
        // changes like a background color/type edit made without switching
        // tabs. That was the reported bug (font size worked because it reads
        // the live global variable directly; background went through this
        // stale-profile path instead).
        return liveBackgroundState;
      }
    ...
    // Sibling of estimateWrappedLineCount() that returns the actual wrapped physical
    // lines (not just a count) — used to split long sermon paragraphs across several
    // display pages (see getPagesFromItem()'s sermon-collection branch below).
    function wrapVerseTextIntoLines(text, maxWidthPx, fontSizePt, fontFamily, fontWeight) {
      const trimmedInput = String(text || '').trim();
      if (!trimmedInput) return [];
      if (!Number.isFinite(maxWidthPx) || maxWidthPx <= 0) return [trimmedInput];
      const canvas = estimateWrappedLineCount._canvas || (estimateWrappedLineCount._canvas = document.createElement('canvas'));
      const ctx = canvas.getContext('2d');
      if (!ctx) return [trimmedInput];
      const sizePx = (Number(fontSizePt) || 0) * (96 / 72);
      const weight = fontWeight || '700';
      ctx.font = `${weight} ${sizePx}px ${fontFamily || 'sans-serif'}`;
      const spaceWidth = ctx.measureText(' ').width || 0;
      ...
      let currentWidth = 0;
      words.forEach((word) => {
        const wordWidth = ctx.measureText(word).width || 0;
        const addWidth = currentWidth === 0 ? wordWidth : spaceWidth + wordWidth;
        if (currentWidth === 0 || currentWidth + addWidth <= maxWidthPx) {
          currentWords.push(word);
          currentWidth += addWidth;
        } else {
          outLines.push(currentWords.join(' '));
          currentWords = [word];
          currentWidth = wordWidth;
        }
      });
      if (currentWords.length) outLines.push(currentWords.join(' '));
      return outLines.length ? outLines : [trimmedInput];
    }

    function estimateFullTextHeightPx(lineCount, fontSizePt, lineHeight, isBible, refFontSizePt, refLineCount = 1) {
      const sizePx = (Number(fontSizePt) || 0) * (96 / 72);
      ...
      const refHeight = (refSizePx * 1.1 * Math.max(1, Number(refLineCount) || 1)) + 20;
