    ...
    function broadcastMessage(msg) {
      ...
      if (isVmixMode() && channel) channel.postMessage(msg);
      // Desktop shell: BroadcastChannel between two separate Electron windows
      // proved unreliable, so also relay every message via IPC as a guaranteed
      // delivery path (see preload.js / main.js). Harmless no-op in the plain
      // browser build where window.BSPDesktop doesn't exist.
      if (window.BSPDesktop && typeof window.BSPDesktop.relayToOutput === 'function') {
        window.BSPDesktop.relayToOutput(msg);
      }
    ...
    function captureLiveRenderUiSnapshot() {
      ...
      // Quick controls (e.g. the font-size stepper next to the settings
      // button) edit the live DOM inputs directly, but onAnyControlChange()
      // saves that edit into the profile for whatever tab is the CURRENT
      // settings target (follow mode -> currently viewed sidebar tab) — which
      // only matches the live kind's own profile when the operator happens to
      // be viewing that same tab (e.g. picking songs from "Schedule" instead
      // of "Songs" saves into the wrong profile). Same root cause as the
      // background-not-updating bug: when the panel is currently showing the
      // live content type, the freshly captured state is authoritative and
      // must win over the (possibly out-of-sync) stored per-tab profile.
      ...
      const liveProjection = viewingLiveTab
        ? captureProjectionSettingsSnapshot()
        : ((typeof getProjectionSettingsSnapshotForTab === 'function')
    ...
    function pushLiveUpdate() {
      ...
      if (shouldAutoExpandBibleLtHeight) {
        ...
        // Long sermon titles can wrap onto a 2nd line in the reference badge — measure
        // it the same way body text is measured, so the LT background/container grows
        // tall enough to fit it instead of clipping the top line off-screen.
        const refTextForMeasure = getBibleRefForPage(liveItem, p.raw, p.verseCount) || '';
        const refWidthPx = Math.max(160, textWidthPx * 0.92);
        const refLines = refTextForMeasure
          ? estimateWrappedLineCount(refTextForMeasure, refWidthPx, ltRefSize, fontFamily, 800)
          : 1;
        dynamicHeight = Math.max(dynamicHeight, calculateLtHeightPctFromLines(targetLines, refLines));
      }
      ...
      if (!useCustomStyle && mode !== 'full' && autoResizeLT !== 'none') {
        ...
        // 18pt used to be a hard floor here, but very long sermon paragraphs (plus a
        // long, possibly 2-line, title) sometimes genuinely need to go smaller than
        // that to fit without overflowing — down to 10pt is still legible and matches
        // what users were doing manually before this floor was lowered.
        const minSize = 10;
        const maxSize = isBibleLT
          ? (dualActive ? dualBibleLtMaxFont : singleBibleLtMaxFont)
          : 120;
        const refLineCount = isBibleLT
          ? (() => {
              const refTextForFit = getBibleRefForPage(liveItem, p.raw, p.verseCount) || '';
              if (!refTextForFit) return 1;
              const refWidthPx = Math.max(160, maxWidthPx * 0.92);
              return Math.max(1, estimateWrappedLineCount(refTextForFit, refWidthPx, ltRefSize, fontFamily, 800));
            })()
          : 1;
        const fitsAt = (sizePt) => {
          const lines = estimateWrappedLineCount(ltText, maxWidthPx, sizePt, fontFamily, fontWeight);
          const height = estimateFullTextHeightPx(lines, sizePt, lineHeightAdjusted, isBibleLT, refFontSize, refLineCount);
      ...
      if (livePointer.kind === 'bible') {
        ...
        // A sermon collection is a single version by nature — showing its
        // internal abbreviation (e.g. "(SB. BSS)") next to every sermon
        // reference is just clutter, unlike an actual multi-version Bible
        // where "(KJV)" vs "(LSG)" is meaningful to the audience.
        ...
        // The title now gets its own dedicated page (see buildSongTitlePage in
        // songs-and-bible.js) shown before the lyrics start, so it must not
        // also float as a reference badge over every lyric page.
        const songReferenceLabel = '';
        const ltRefSize = ltRefFontSize || refFontSize;
        if (mode !== 'full' && useCustomStyle) {
          outHtml = `<div class="jo-body" style="padding-left:60px;${verseShadowStyle}">${songPrimaryHtml}</div>`;
        } else if (mode === 'full') {
          outHtml = songReferenceLabel
            ? buildFullBibleSegment({
                referenceLabel: songReferenceLabel,
                verseHtml: songPrimaryHtml,
                refSize: refFontSize,
                refColor,
                refBgColor,
                verseAlign: 'center',
            ...
            : `<div class="jo-body" style="text-align:center;${verseShadowStyle}">${songPrimaryHtml}</div>`;
        } else {
          outHtml = songReferenceLabel
            ? buildLtBibleSegment({
                referenceLabel: songReferenceLabel,
            ...
            : `<div class="jo-body" style="${verseShadowStyle}">${songPrimaryHtml}</div>`;
        }
