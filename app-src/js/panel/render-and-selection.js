    ...
    // Sermon book names are stored as "Title (dateCode)" (the parenthetical date code
    // keeps otherwise-identical titles unique — see the bulk sermon import). For display
    // (projection reference, recent/pinned references) this reads better as an em-dash:
    // "Title — dateCode", matching the Little Storehouse / Grenier du Message convention.
    function formatSermonBookLabel(book) {
    ...
    function updateBibleLists() {
      ...
      if (!activeBibleVersion || !bibles[activeBibleVersion]) {
        updateSermonGridButtonVisibility();
        return;
      }
    ...
    function updateSermonGridButtonVisibility() {
      const btn = document.getElementById('bible-sermon-grid-btn');
      if (!btn) return;
      btn.style.display = isActiveSermonCollection() ? 'inline-flex' : 'none';
    }

    // Finds all paragraph numbers of a sermon "book" (its unique chapterItem, chapter
    // fixed at "1") and jumps the current selection directly to the given paragraph —
    // shared by handleChapterSelect(), the paragraph grid, and search results.
    function getSermonParagraphNumbers(book) {
      if (!book || !activeBibleVersion || !bibles[activeBibleVersion]) return [];
      const item = bibles[activeBibleVersion].find(c => (c.book || extractBookAndChapter(c).book) === book);
      if (!item) return [];
      const paragraphs = [];
    ...
    // Keeps the book/paragraph selectors (and, critically, the sidebar's book filter —
    // see renderSongs()) in sync with whatever sermon is actually selected. Without this,
    // any selection made outside handleBookSelect() (search results, the paragraph grid,
    // the post-import fallback selection) leaves #bible-book-select blank, which makes
    // the sidebar list fall back to rendering EVERY book in the collection on every
    // render instead of just the current one — the real cause of both the "paragraph
    // selector disappears after import" and the general sluggishness reports.
    function syncSermonNavSelectors() {
      if (!isActiveSermonCollection() || !currentItem || !getIsBibleItem(currentItem)) return;
      const bookSelect = document.getElementById('bible-book-select');
      const chapSelect = document.getElementById('bible-chap-select');
      if (!bookSelect || !chapSelect) return;
      const book = currentItem.book || extractBookAndChapter(currentItem).book;
      if (!book) return;
      if (bookSelect.value !== book) bookSelect.value = book;
      chapSelect.innerHTML = `<option value="">${esc(t('ui_paragraph_short'))}</option>`;
      chapSelect.disabled = false;
      getSermonParagraphNumbers(book).forEach((p) => {
        const opt = document.createElement('option');
        opt.value = p;
        opt.textContent = p;
      ...
      const currentParagraph = getFirstVerseNumber(pages[lineCursor]?.raw);
      if (currentParagraph) chapSelect.value = currentParagraph;
    }

    function jumpToSermonParagraph(book, paragraph) {
      if (!book || !paragraph || !activeBibleVersion || !bibles[activeBibleVersion]) return;
      const idx = bibles[activeBibleVersion].findIndex(c => (c.book || extractBookAndChapter(c).book) === book);
      if (idx === -1) return;
      selectItem(idx, { preserveLineCursor: true, skipButtonView: true });
      setBibleGroupAnchor(paragraph, currentItem);
      const pages = getPagesFromItem(currentItem, true);
      const vIdx = pages.findIndex(p => matchesVerseStart(p.raw, paragraph));
      if (vIdx !== -1) lineCursor = vIdx;
      updateButtonView();
      addBibleRecentReference(buildBibleRefEntry(book, '1', paragraph, null, activeBibleVersion));
      const bookSelect = document.getElementById('bible-book-select');
      const chapSelect = document.getElementById('bible-chap-select');
      if (bookSelect && bookSelect.value !== book) bookSelect.value = book;
      if (chapSelect) chapSelect.value = String(paragraph);
    }

    function openSermonParagraphGrid() {
      if (!isActiveSermonCollection()) return;
      const bookSelect = document.getElementById('bible-book-select');
      const book = (bookSelect && bookSelect.value)
        || (currentItem ? (currentItem.book || extractBookAndChapter(currentItem).book) : '');
      if (!book) return;
      const paragraphs = getSermonParagraphNumbers(book);
      if (!paragraphs.length) return;
      const grid = document.getElementById('sermon-paragraph-grid');
      const titleEl = document.getElementById('sermon-paragraph-grid-title');
      if (!grid) return;
      if (titleEl) titleEl.textContent = book;
      grid.innerHTML = '';
      const currentParagraph = currentItem && lineCursor != null
        ? getFirstVerseNumber(getPagesFromItem(currentItem, true)[lineCursor]?.raw)
        : null;
      paragraphs.forEach((p) => {
        const cell = document.createElement('div');
        cell.className = 'sermon-paragraph-cell';
        if (currentParagraph === p) cell.classList.add('active');
        cell.textContent = p;
        cell.onclick = () => {
          closeModal('sermonParagraphGridModal');
          jumpToSermonParagraph(book, p);
        };
    ...
    function isActiveSermonCollection() {
      return !!(activeBibleVersion && bibleVersionMeta[activeBibleVersion] && bibleVersionMeta[activeBibleVersion].collectionType === 'sermon');
    }
    ...
    function handleBookSelect() {
      ...
      const isSermonCollection = isActiveSermonCollection();

      chapSelect.innerHTML = `<option value="">${esc(t(isSermonCollection ? 'ui_paragraph_short' : 'ui_chapter_short'))}</option>`;
      ...
      if (isSermonCollection) {
        // A sermon has a single fixed "chapter" (always "1") but many numbered
        // paragraphs — repurpose this dropdown to list paragraph numbers instead.
        getSermonParagraphNumbers(book).forEach((p) => {
          const opt = document.createElement('option');
          opt.value = p;
          opt.textContent = p;
    ...
    function handleChapterSelect() {
      ...
      if (isActiveSermonCollection()) {
        jumpToSermonParagraph(book, chap);
        return;
      }
    ...
    function renderBibleSearchResults(qRaw) {
      ...
      const isSermonCollection = !!(bibleVersionMeta[activeBibleVersion] && bibleVersionMeta[activeBibleVersion].collectionType === 'sermon');
      const bibleList = (activeBibleVersion && bibles[activeBibleVersion]) ? bibles[activeBibleVersion] : [];

      // For sermon collections, a title search should surface each matching sermon once
      // (not once per paragraph that happens to mention it). Typing a trailing number
      // ("Crois Seulement 12" or "61-0427 12") jumps straight to that paragraph instead
      // of paragraph 1 — sermon book names contain digits/parentheses so they can't use
      // the strict Bible "book chapter:verse" reference syntax.
      const sermonBookParaQuery = isSermonCollection && !referenceQuery ? parseSermonBookAndParagraphQuery(qRaw) : null;
      let bookMatches = [];
      let skipKeywordSearch = false;
      if (sermonBookParaQuery) {
        const candidates = findSermonBookMatches(sermonBookParaQuery.bookQuery, { versionId: activeBibleVersion, maxResults: 30 });
        if (candidates.length) {
          bookMatches = candidates.map((c) => {
            const item = bibleList[c.chapterIndex];
            const para = findSermonParagraphByNumber(item, sermonBookParaQuery.paragraph);
            return { ...c, targetVerse: sermonBookParaQuery.paragraph, previewText: para ? para.text : '' };
          });
          skipKeywordSearch = true;
        }
      }
      if (!bookMatches.length && isSermonCollection && !referenceQuery) {
        bookMatches = findSermonBookMatches(qRaw, { versionId: activeBibleVersion, maxResults: 200 })
          .map((c) => ({ ...c, targetVerse: '1', previewText: '' }));
        skipKeywordSearch = true;
      }
      // For sermon collections, this sidebar list is title-only by design — the
      // separate content/paragraph search lives in the nav-mirror search box next to
      // Project Live (see buildSermonContentMatchItems / bindNavMirrorSearch), so
      // keyword/paragraph matches are deliberately never mixed in here.
      const results = skipKeywordSearch
        ? []
        : (referenceQuery
      ...
      const formatResultLabel = (book, chapter, verse) => isSermonCollection
        ? `${formatSermonBookLabel(book)} §${verse}`
        : `${book} ${chapter}:${verse}`;
      const formatBookMatchLabel = (book, targetVerse) => (targetVerse && targetVerse !== '1')
        ? `${formatSermonBookLabel(book)} §${targetVerse}`
        : formatSermonBookLabel(book);

      const totalCount = bookMatches.length + results.length;
      if (currentSearchPos != null && (currentSearchPos < 0 || currentSearchPos >= totalCount)) {
      ...
      const bookMirrorItems = bookMatches.map((entry, idx) => {
        const sourceItem = bibleList[entry.chapterIndex] || null;
        return {
          title: formatBookMatchLabel(entry.book, entry.targetVerse),
          snippet: entry.previewText || '',
          active: currentSearchPos === idx,
          activate: () => {
            currentSearchPos = idx;
            activateBibleSearchResult(entry.chapterIndex, entry.targetVerse || '1');
          },
          contextMenu: (clientX, clientY) => {
            if (typeof window._openSidebarContextMenu !== 'function' || !sourceItem) return;
            window._openSidebarContextMenu('bible', sourceItem, entry.chapterIndex, idx, clientX, clientY, {});
          }
        };
      });
      const resultMirrorItems = results.map((entry, resultIndex) => {
        const pos = bookMatches.length + resultIndex;
        const sourceItem = bibleList[entry.chapterIndex] || null;
        return {
          title: formatResultLabel(entry.book, entry.chapter, entry.verse),
          snippet: entry.text || '',
          active: currentSearchPos === pos,
          activate: () => {
          ...
          contextMenu: (clientX, clientY) => {
            if (typeof window._openSidebarContextMenu !== 'function' || !sourceItem) return;
            window._openSidebarContextMenu('bible', sourceItem, entry.chapterIndex, pos, clientX, clientY, { bibleVerse: entry.verse });
          }
        };
      });
      list._navMirrorItems = [...bookMirrorItems, ...resultMirrorItems];

      bookMatches.forEach((entry, idx) => {
        const targetVerse = entry.targetVerse || '1';
        const div = document.createElement('div');
        div.className = 'song-item';
        if (currentSearchPos === idx) div.classList.add('active');
        div.dataset.index = String(entry.chapterIndex);
        div.dataset.chapterIndex = String(entry.chapterIndex);
        div.dataset.verse = targetVerse;
        div.dataset.pos = String(idx);
        const left = document.createElement('div');
        const title = document.createElement('span');
        title.className = 'search-title';
        title.textContent = `📖 ${formatBookMatchLabel(entry.book, targetVerse)}`;
        left.appendChild(title);
        if (entry.previewText) {
          const snippet = document.createElement('span');
          snippet.className = 'search-snippet';
          snippet.textContent = entry.previewText;
          left.appendChild(snippet);
        }
        div.appendChild(left);
        div.onclick = () => {
          currentSearchPos = idx;
          activateBibleSearchResult(entry.chapterIndex, targetVerse);
        };
        div.ondblclick = () => {
          currentSearchPos = idx;
          activateBibleSearchResult(entry.chapterIndex, targetVerse);
        ...
        div.addEventListener('contextmenu', (e) => {
          ...
          window._openSidebarContextMenu('bible', sourceItem, entry.chapterIndex, idx, e.clientX, e.clientY, {});
        });
      ...
      results.forEach((entry, resultIndex) => {
        ...
        const div = document.createElement('div');
        div.className = 'song-item';
        if (currentSearchPos === pos) div.classList.add('active');
        div.dataset.index = String(entry.chapterIndex);
        div.dataset.chapterIndex = String(entry.chapterIndex);
        ...
        const title = document.createElement('span');
        title.className = 'search-title';
        title.textContent = formatResultLabel(entry.book, entry.chapter, entry.verse);
        const snippet = document.createElement('span');
        snippet.className = 'search-snippet';
    ...
    // Independent content/paragraph search for sermon collections, used by the
    // nav-mirror search box next to Project Live — deliberately separate from the
    // sidebar's title-only search above, per user request, so the two inputs each do
    // one job instead of mixing titles and paragraph snippets together.
    function buildSermonContentMatchItems(qRaw) {
      if (!activeBibleVersion || !bibles[activeBibleVersion]) return [];
      const bibleList = bibles[activeBibleVersion] || [];
      const q = normalizeSearchText(qRaw || '').trim();
      if (q.length < 2) return [];
      const results = findBibleKeywordMatches(qRaw, {
        versionId: activeBibleVersion,
        book: '',
        maxResults: 200,
        rankAndExpand: true
      ...
      return results.map((entry) => {
        const sourceItem = bibleList[entry.chapterIndex] || null;
        return {
          title: `${formatSermonBookLabel(entry.book)} §${entry.verse}`,
          snippet: entry.text || '',
          active: false,
          activate: () => activateBibleSearchResult(entry.chapterIndex, entry.verse),
          contextMenu: (clientX, clientY) => {
            if (typeof window._openSidebarContextMenu !== 'function' || !sourceItem) return;
            window._openSidebarContextMenu('bible', sourceItem, entry.chapterIndex, 0, clientX, clientY, { bibleVerse: entry.verse });
          }
        };
      });
    }
    ...
    const DEFAULT_SERMON_VERSION_NAME = 'Sermons Branham (FR)';

    // Called from the native "File" menu (see main.js) after the user picks a
    // file via the OS file dialog and it's been read to text in the main
    ...
    // Shared by drag-and-drop import (handleImport) and the native File menu's
    // re-import button, so both paths behave identically for the same file.
    // Returns true if the JSON was recognized (song or sermon collection),
    // false otherwise (caller should count it as unsupported).
    function importBulkJsonText(text) {
      let bulkData = null;
      try { bulkData = JSON.parse(text); } catch (e) { bulkData = null; }
      if (bulkData && bulkData.type === 'bsp-song-collection' && Array.isArray(bulkData.songs)) {
        const incomingSongs = bulkData.songs.filter(s => s && s.title && (s.text || s.content));
        const songBulkMessage = t('song_bulk_import_confirm_message')
          .replace('{count}', String(incomingSongs.length));
        showConfirm(t('song_bulk_import_confirm_title'), songBulkMessage, async () => {
          const existingTitles = new Set(songs.map(s => normalizeSearchText(s.title || '')));
          let addedCount = 0;
          incomingSongs.forEach((s) => {
            const titleKey = normalizeSearchText(s.title || '');
            if (existingTitles.has(titleKey)) return;
            existingTitles.add(titleKey);
            const songText = s.text || s.content || '';
            const newSong = {
              id: createId('song', s.title),
              title: s.title,
              content: songText,
              text: songText,
              translatedLyrics: '',
              ...
              translatedAt: 0,
              translationHash: '',
              searchableText: normalizeSearchText(`${s.title}\n${songText}`),
              createdAt: Date.now(),
              updatedAt: Date.now()
            };
            songs.push(newSong);
          ...
          try {
            await flushAppState();
            showToast(t('song_bulk_imported').replace('{count}', String(addedCount)));
          } catch (e) {
            console.error('Song bulk import persist failed', e);
            showToast(t('sermon_import_persist_failed'));
          }
        });
        return true;
      }
      if (!bulkData || bulkData.type !== 'bsp-sermon-collection' || !Array.isArray(bulkData.books)) {
        return false;
      }
      const incoming = bulkData.books.filter(b => b && b.book && b.content);
      const bulkMessage = t('sermon_bulk_import_confirm_message')
        .replace('{count}', String(incoming.length))
        .replace('{version}', String(bulkData.version || DEFAULT_SERMON_VERSION_NAME));
      showConfirm(t('sermon_bulk_import_confirm_title'), bulkMessage, async () => {
        const versionName = String(bulkData.version || '').trim() || DEFAULT_SERMON_VERSION_NAME;
        const existingList = bibles[versionName] || [];
        const isNewVersion = !existingList.length;
        const dedupedExisting = existingList.filter(c =>
          !incoming.some(b => normalizeBookName(b.book) === normalizeBookName(c.book))
        );
        bibles[versionName] = [...dedupedExisting, ...incoming.map(b => ({ ...b, version: versionName }))];
        clearBibleSearchCache(versionName);
        bibleVersionMeta[versionName] = { collectionType: 'sermon' };
        if (!activeBibleVersion || !bibles[activeBibleVersion]) activeBibleVersion = versionName;
        renderVersionBar();
        updateBibleLists();
        renderSongs();
        ensureSelectionFallback();
        ...
        saveToStorageDebounced();
        try {
          await idbPut(STORE_BIBLES, buildBibleRecord(versionName, bibles[versionName], { isNew: isNewVersion, collectionType: 'sermon' }));
          await flushAppState();
          showToast(t('sermon_bulk_imported').replace('{count}', String(incoming.length)));
        } catch (e) {
          console.error('Sermon bulk import persist failed', e);
          showToast(t('sermon_import_persist_failed'));
        }
      });
      return true;
      ...
      for (const file of input.files) {
        try {
          ...
          if (name.endsWith('.xml')) {
            ...
            const parsed = await parseSermonPdfFile(file);
            if (!parsed.paragraphCount) {
              showToast(t('sermon_import_no_paragraphs'));
              skippedUnsupported += 1;
              continue;
            }
            const previewMessage = t('sermon_import_confirm_message')
              .replace('{book}', parsed.book)
              .replace('{count}', String(parsed.paragraphCount))
              .replace('{first}', parsed.firstParagraph ? parsed.firstParagraph.text.slice(0, 90) : '')
              .replace('{last}', parsed.lastParagraph ? parsed.lastParagraph.text.slice(0, 90) : '');
            showConfirm(t('sermon_import_confirm_title'), previewMessage, async (versionNameInput) => {
              const versionName = String(versionNameInput || '').trim() || DEFAULT_SERMON_VERSION_NAME;
              const chapterItem = { title: parsed.title, book: parsed.book, chapter: parsed.chapter, content: parsed.content, version: versionName };
              const existingList = bibles[versionName] || [];
              const isNewVersion = !existingList.length;
              const dedupedList = existingList.filter(c => normalizeBookName(c.book) !== normalizeBookName(parsed.book));
              bibles[versionName] = [...dedupedList, chapterItem];
              clearBibleSearchCache(versionName);
              bibleVersionMeta[versionName] = { collectionType: 'sermon' };
              if (!activeBibleVersion || !bibles[activeBibleVersion]) activeBibleVersion = versionName;
              renderVersionBar();
              ...
              try {
                await idbPut(STORE_BIBLES, buildBibleRecord(versionName, bibles[versionName], { isNew: isNewVersion, collectionType: 'sermon' }));
                await flushAppState();
                showToast(t('sermon_imported'));
              } catch (e) {
                console.error('Sermon import persist failed', e);
                showToast(t('sermon_import_persist_failed'));
              }
            }, true, DEFAULT_SERMON_VERSION_NAME);
          } else if (name.endsWith('.json')) {
            if (!importBulkJsonText(text)) skippedUnsupported += 1;
    ...
    function coerceBibleRecord(raw) {
      ...
      if (Array.isArray(raw.parsedData)) {
        ...
        if (raw.collectionType) record.collectionType = raw.collectionType;
        return record;
      }
      if (raw.xmlText) {
        try {
    ...
    async function exportBackup() {
      try {
        ...
        if (!bibleRecords.length && Object.keys(bibles).length) {
          bibleRecords = Object.keys(bibles).map(name => buildBibleRecord(name, bibles[name] || [], {
            isNew: false,
            collectionType: (bibleVersionMeta[name] && bibleVersionMeta[name].collectionType) || null
          })).filter(Boolean);
        }
