...
<head>
  ...
  <title>Message WMB Worship Pro - ${opts.preview ? 'Program Preview' : 'Standalone Output'}</title>
  <style>
...
<body>
  ...
  <\/script>
    ...
    // Single toolbar button (desktop shell) toggling the projection window on/off,
    // wrapping the existing startOutputLive()/endOutputLive() pair.
    function toggleOutputLive() {
      if (outputLiveActive) {
        endOutputLive();
      } else {
        startOutputLive();
