    ...
    function getCurrentBibleRefFromView() {
      ...
      const refVersionId = activeBibleVersion || null;
      const isSermon = !!(refVersionId && bibleVersionMeta[refVersionId] && bibleVersionMeta[refVersionId].collectionType === 'sermon');
      const displayTitle = isSermon && verseStart
        ? `${formatSermonBookLabel(book)} §${verseStart}${(verseEnd && verseEnd !== verseStart) ? `-${verseEnd}` : ''}`
        : query;
      return {
        query,
        title: displayTitle,
    ...
    function buildBibleRefEntry(book, chapter, verseStart, verseEnd, versionId) {
      ...
      const refVersionId = versionId || activeBibleVersion || null;
      const isSermon = !!(refVersionId && bibleVersionMeta[refVersionId] && bibleVersionMeta[refVersionId].collectionType === 'sermon');
      const displayTitle = isSermon && vStart
        ? `${formatSermonBookLabel(cleanBook)} §${vStart}${(vEnd && vEnd !== vStart) ? `-${vEnd}` : ''}`
        : query;
      return {
        query,
        title: displayTitle,
