    ...
    function findBibleKeywordMatches(query, opts = {}) {
      ...
      // rankAndExpand: used for large collections (e.g. sermon corpora) where a plain
      // "first N matches in array order" would only ever surface hits from the very
      // start of the collection for a common word. Ranks by how often the query terms
      // occur instead of stopping at the first maxResults hits — but still caps how many
      // candidates get scored/sorted (a very common word can match tens of thousands of
      // paragraphs; scoring+sorting all of them on every keystroke would be the search
      // box itself, not just the sermon feature, feeling slow).
      if (opts.rankAndExpand) {
        const scanCap = Math.max(maxResults * 10, 3000);
        const scored = [];
        for (const entry of entries) {
          if (bookKey && normalizeBookName(entry.book) !== bookKey) continue;
          if (!tokens.every(t => entry.searchText.includes(t))) continue;
          const score = tokens.reduce((sum, t) => sum + countTokenOccurrences(entry.searchText, t), 0);
    ...
    // One result per matching "book" (sermon), not per paragraph — for browsing a large
    // sermon collection by title rather than flooding results with every paragraph that
    // happens to mention the title text.
    function findSermonBookMatches(query, opts = {}) {
      const q = normalizeSearchText(query).trim();
      if (!q || q.length < 2) return [];
      const versionId = opts.versionId || activeBibleVersion;
      if (!versionId || !bibles[versionId]) return [];
      ...
      // Tiered matching: a plain "does the title contain this substring
      // anywhere" search surfaces every sermon that happens to share a common
      // word ("beaucoup de sermons" for something short/common). Prefer the
      // tightest tier that actually found something, so a query that clearly
      // identifies one sermon isn't buried under loose partial matches.
      const exact = [];
      const prefix = [];
      const wordPrefix = [];
      const contains = [];
      for (let i = 0; i < list.length; i += 1) {
        ...
        const normBook = normalizeSearchText(book);
        if (!normBook.includes(q)) continue;
        const entry = { chapterIndex: i, book, chapter: item?.chapter || '1' };
        if (normBook === q) exact.push(entry);
        else if (normBook.startsWith(q)) prefix.push(entry);
        else if (normBook.split(' ').some(w => w.startsWith(q))) wordPrefix.push(entry);
        else contains.push(entry);
      }
      const bestTier = [exact, prefix, wordPrefix, contains].find(tier => tier.length);
      return bestTier ? bestTier.slice(0, maxResults) : [];
    }

    // "Crois Seulement 12" or "61-0427 12" -> { bookQuery: "Crois Seulement", paragraph: "12" }.
    // Sermon book names contain digits/parentheses (date codes), so they can't go through
    // parseBibleReferenceQuery's strict "book chapter[:verse]" regex — this is a looser,
    // sermon-specific parser that just looks for a trailing paragraph number.
    function parseSermonBookAndParagraphQuery(raw) {
      const trimmed = String(raw || '').trim();
      const match = trimmed.match(/^(.+?)\s+(\d{1,4})$/);
      if (!match) return null;
      const bookQuery = match[1].trim();
      if (!bookQuery) return null;
      return { bookQuery, paragraph: match[2] };
    }

    // Finds a specific numbered paragraph's text within a sermon chapterItem's content.
    function findSermonParagraphByNumber(item, paragraphNum) {
    ...
    function buildBibleRecord(name, parsedData, { isNew = false, createdAt = null, collectionType = null } = {}) {
      const now = Date.now();
      const id = name;
      ...
      if (collectionType) record.collectionType = collectionType;
      return record;
    }

    function persistBibleVersion(name) {
      const data = bibles[name] || [];
      const collectionType = (bibleVersionMeta[name] && bibleVersionMeta[name].collectionType) || null;
      const record = buildBibleRecord(name, data, { collectionType });
      return idbPut(STORE_BIBLES, record);
    }

    const persistBibleVersionTimers = new Map();
    // Coalesces rapid successive calls (e.g. highlighting several words in a row) into
    // a single write. Matters most for large collections (a sermon corpus can be 100MB+)
    // where persistBibleVersion() re-serializes the WHOLE version on every call — without
    // this, each small edit would trigger a full multi-second IndexedDB write.
    function schedulePersistBibleVersion(name) {
      if (!name) return;
      const existing = persistBibleVersionTimers.get(name);
      if (existing) clearTimeout(existing);
      const timer = setTimeout(() => {
        persistBibleVersionTimers.delete(name);
        persistBibleVersion(name).catch(() => {});
      }, 1500);
      persistBibleVersionTimers.set(name, timer);
    }
    ...
    // Background images/videos are now referenced by a lightweight file://
    // URL (see getDesktopFileUrl() in media-and-background.js) instead of
    // being read into memory as base64 — that base64 approach is what used
    // to freeze/crash the app on "heavy" video backgrounds. A profile saved
    // before that fix may still have an oversized base64 blob stored; drop
    // it on load instead of pulling it back into memory (and re-triggering
    // the same crash) on every launch.
    const BG_IMAGE_MAX_DATA_URL_CHARS = 34 * 1024 * 1024; // ~25MB decoded
    const BG_VIDEO_MAX_DATA_URL_CHARS = 134 * 1024 * 1024; // ~100MB decoded
    function sanitizeStoredBgMediaUrl(value, maxDataUrlChars) {
      if (typeof value !== 'string' || !value) return value || null;
      if (value.startsWith('data:') && value.length > maxDataUrlChars) return null;
      return value;
    }
    ...
    // Background is meant to be one shared setup, not something you configure
    // three times over — but it lived inside the same per-tab (Bible/Songs/
    // Setlist) settings profile as everything else, so switching that tab
    // swapped in a DIFFERENT, independently-tracked background config. That's
    // why a video background "worked in Bible but not in Songs": the Songs
    // profile's own bgType had simply never been set to 'video'. These keys
    // are kept identical across all three profiles instead (see
    // syncBackgroundKeysAcrossProfiles below).
    const BACKGROUND_SHARED_KEYS = [
      'bgType', 'bgImageSource', 'bgImageUrl', 'bgUploadDataUrl',
      'bgVideoSource', 'bgVideoUrl', 'bgVideoUploadDataUrl', 'bgVideoLoop', 'bgVideoSpeed',
      'bgMode', 'bgColor', 'bgGradientShadow', 'bgGradientHighlight', 'bgGradientAngle',
      'bgBlur', 'bgEdgeFix', 'bgOpacity', 'bgOpacityFull', 'bgOpacityLT', 'bgY', 'bgToggle'
    ];

    function syncBackgroundKeysAcrossProfiles(sourceTab) {
      if (!projectionSettingsProfilesByTab || typeof projectionSettingsProfilesByTab !== 'object') return;
      const source = projectionSettingsProfilesByTab[sourceTab];
      if (!source) return;
      SETTINGS_TARGET_TABS.forEach((tab) => {
        if (tab === sourceTab) return;
        const target = projectionSettingsProfilesByTab[tab];
        if (!target) return;
        BACKGROUND_SHARED_KEYS.forEach((key) => {
    ...
    function loadProjectionSettingsProfilesFromUi(ui) {
      ...
      // One-time reconciliation for profiles saved before background settings
      // became shared: if they've diverged, prefer whichever one was actually
      // configured (bgType other than the 'color' default) so a background
      // already set up under one tab isn't silently lost for the others.
      const configuredTab = SETTINGS_TARGET_TABS.find((tab) => {
        const p = projectionSettingsProfilesByTab[tab];
        return p && p.bgType && p.bgType !== 'color';
      });
      syncBackgroundKeysAcrossProfiles(configuredTab || 'bible');
      return projectionSettingsProfilesByTab;
    }
    ...
    function saveProjectionSettingsProfileForTab(tab = getEffectiveSettingsTargetTab()) {
      ...
      syncBackgroundKeysAcrossProfiles(tab);
      return profiles[tab];
    }
    ...
    function applyProjectionSettingsSnapshot(profile, opts = {}) {
      ...
      if (Object.prototype.hasOwnProperty.call(next, 'bgUploadDataUrl')) bgUploadDataUrl = sanitizeStoredBgMediaUrl(next.bgUploadDataUrl, BG_IMAGE_MAX_DATA_URL_CHARS);
      ...
      if (Object.prototype.hasOwnProperty.call(next, 'bgVideoUploadDataUrl')) bgVideoUploadDataUrl = sanitizeStoredBgMediaUrl(next.bgVideoUploadDataUrl, BG_VIDEO_MAX_DATA_URL_CHARS);
